...
# rates spread with sigma = 0.1 on both p_gen and p_swap (variance-matched
# uniform, clipped to [0.05,1]). Companion to submit_dvN_infmem.sh (sigma=0).
# Merge locally with
#   python experiments/comparisons/merge_json.py \
#     'results/comparisons/dvN_cinf_std0.1_H40000/*.json' \
#     -o results/comparisons/delivery_vs_N_cinf_std0.1_H40000.json

set -euo pipefail
cd "$SLURM_SUBMIT_DIR"
mkdir -p slurm_logs results/comparisons/dvN_cinf_std0.1_H40000
...
python -u experiments/comparisons/policy_vs_agent/delivery_vs_N.py \
    ...
    --out "results/comparisons/dvN_cinf_std0.1_H40000/N${N}_${pol}.json"
