...
python -u src/rl_stack/teacher_student/train_student.py \
...
python -u src/rl_stack/teacher_student/eval_student.py \
    --student "$OUT/policy.pth" --teacher "$TEACHER" \
    --n_lo 4 --n_hi 12 --n_ch 4 --p_gen 0.4 --p_swap 0.8 --cutoff 20 \
    --mc_eps 1000 --horizon 2000 \
    --out "$JSON" --fig "$FIG"
python -u src/rl_stack/teacher_student/eval_student.py --plot --out "$JSON" --fig "$FIG"
