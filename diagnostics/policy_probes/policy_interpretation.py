...
def plot_swap_preference(model, save_dir=".", resolution=40, device="cpu"):
    ...
    """
    N = 5
    T_REM = 0.5  # mid-episode time remaining
    ...
    for i, f1 in enumerate(f_range):
        for j, f2 in enumerate(f_range):
            ...
            # Source (node 0): has one link, source can't swap/purify
            features[0] = [0.25, f1, 1, 0, 0.25, 0, 0, T_REM]
            # Node 1: intermediate, 2 occupied & available → can_swap
            features[1] = [0.5, (f1 + 0.5) / 2, 0, 0, 0.5, 1, 0, T_REM]
            # Node 2 (probe): 2 links to different neighbours → can_swap
            mean_f = (f1 + f2) / 2.0
            features[2] = [0.5, mean_f, 0, 0, 0.5, 1, 0, T_REM]
            # Node 3: intermediate, 2 occupied & available → can_swap
            features[3] = [0.5, (f2 + 0.5) / 2, 0, 0, 0.5, 1, 0, T_REM]
            # Dest (node 4): dest can't swap/purify
            features[4] = [0.25, f2, 0, 1, 0.25, 0, 0, T_REM]
...
def plot_purify_preference(model, save_dir=".", resolution=40, device="cpu"):
    ...
    for i, f1 in enumerate(f_range):
        for j, f2 in enumerate(f_range):
            features = np.zeros((N, 8), dtype=np.float32)
            mean_f = (f1 + f2) / 2.0
            # Node 0 (source): 1 occupied link
            features[0] = [0.25, 0.5, 1, 0, 0.25, 0, 0, T_REM]
            # Node 1 (probe): 2 links to same neighbour → can_purify
            features[1] = [0.5, mean_f, 0, 0, 0.5, 0, 1, T_REM]
            # Node 2: intermediate with some links
            features[2] = [0.5, mean_f, 0, 0, 0.5, 1, 0, T_REM]
            # Node 3: intermediate
            features[3] = [0.25, 0.5, 0, 0, 0.25, 0, 0, T_REM]
            # Node 4 (dest): no occupied qubits is unlikely; give it one link
            features[4] = [0.25, 0.5, 0, 1, 0.25, 0, 0, T_REM]
...
def plot_best_action_map(model, save_dir=".", resolution=30, device="cpu"):
    ...
    for i, occ in enumerate(occ_range):
        for j, fid in enumerate(fid_range):
            # frac_available <= frac_occupied; assume all occupied are available
            avail = occ
            can_swap = 1.0 if avail >= 0.5 else 0.0
            can_pur = 1.0 if avail >= 0.5 else 0.0
            features = np.zeros((N, 8), dtype=np.float32)
            features[0] = [0.25, 0.6, 1, 0, 0.25, 0, 0, T_REM]
            features[1] = [occ, fid, 0, 0, avail, can_swap, can_pur, T_REM]
            features[2] = [occ, fid, 0, 0, avail, can_swap, 0, T_REM]
            features[3] = [occ, fid, 0, 0, avail, can_swap, 0, T_REM]
            features[4] = [0.25, 0.6, 0, 1, 0.25, 0, 0, T_REM]
...
def plot_swap_vs_wait(model, save_dir=".", resolution=40, device="cpu"):
    ...
    for i, avail in enumerate(avail_range):
        for j, fid in enumerate(fid_range):
            ...
            features = np.zeros((N, 8), dtype=np.float32)
            features[0] = [0.25, 0.6, 1, 0, 0.25, 0, 0, T_REM]
            features[1] = [occ, fid, 0, 0, avail, can_swap, 0, T_REM]
            features[2] = [occ, fid, 0, 0, avail, can_swap, 0, T_REM]
            features[3] = [occ, fid, 0, 0, avail, can_swap, 0, T_REM]
            features[4] = [0.25, 0.6, 0, 1, 0.25, 0, 0, T_REM]
