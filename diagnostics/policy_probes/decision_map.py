...
Dual-mode so the figure re-renders without recomputing:
  compute: PYTHONPATH=. python diagnostics/policy_probes/decision_map.py --ckpt <path>
  plot:    PYTHONPATH=. python diagnostics/policy_probes/decision_map.py --plot --save_dir <dir>
"""
...
def parse_args():
    ...
    ap.add_argument("--fid_lo", type=float, default=0.5,
                    help="lower edge of the fidelity axis (0.5 = Werner "
                         "separability; below it links are discarded anyway)")
...
def run_compute(a, out_json):
    ...
    data = dict(ckpt=a.ckpt, episodes=a.episodes, seed=a.seed,
                fid_bins=a.fid_bins, fid_lo=a.fid_lo, min_count=a.min_count,
                rows={})
    edges = np.linspace(a.fid_lo, 1.0, a.fid_bins + 1)
    for n_ch in a.n_chs:
...
def run_plot(data, stem):
    ...
    for r, n_ch in enumerate(n_chs):
        ...
        axes[r, 0].set_ylabel("mean fidelity of available qubits")
...
def main():
    ...
    out_json = os.path.join(out, "decision_map.json")
    data = json.load(open(out_json)) if a.plot else run_compute(a, out_json)
    run_plot(data, os.path.join(out, "decision_map"))
